...
func shouldRedistribute(instance *v2beta1.Experiment) bool {
	experimentType := instance.TestingPattern()
	if experimentType == v2beta1.TestingPatternSLOValidation && len(instance.Spec.Versions) == 1 {
		return false
	}
	algorithm := instance.Spec.GetDeploymentPattern()
	return algorithm != v2beta1.DeploymentPatternFixedSplit
}

func redistributeWeight(ctx context.Context, instance *v2beta1.Experiment, restCfg *rest.Config) error {
	log := Logger(ctx)
	log.Info("redistributeWeight called")
	defer log.Info("redistributeWeight ended")

	if !shouldRedistribute(instance) {
		log.Info("No weight redistribution", "strategy", *instance.Status.TestingPattern, "algorithm", instance.Spec.GetDeploymentPattern())
		return nil
	}
...
func addPatch(ctx context.Context, instance *v2beta1.Experiment, version v2beta1.VersionDetail, patcheMap *map[corev1.ObjectReference][]patchIntValue) error {
	log := Logger(ctx)
...
func getCurrentWeight(version string, weights []v2beta1.WeightData) *int32 {
	zero := int32(0)
...
func updateObservedWeights(ctx context.Context, instance *v2beta1.Experiment, restCfg *rest.Config) error {
	log := Logger(ctx)
	...
	observedWeights := make([]v2beta1.WeightData, 0)
	missing := []string{}
	total := int32(0)
	...
	if b.WeightObjRef != nil {
		...
		observedWeights = append(observedWeights, v2beta1.WeightData{Name: b.Name, Value: *w})
		total += *w
	...
	for _, c := range instance.Spec.VersionInfo.Candidates {
		if c.WeightObjRef != nil {
			...
			observedWeights = append(observedWeights, v2beta1.WeightData{Name: c.Name, Value: *w})
			total += *w
	...
	if len(missing) == 1 {
		...
		w := int32(100) - total
		observedWeights = append(observedWeights, v2beta1.WeightData{Name: missing[0], Value: w})
		...
		if *instance.Spec.Strategy.DeploymentPattern != v2beta1.DeploymentPatternFixedSplit {
