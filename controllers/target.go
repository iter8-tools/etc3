...
func (r *ExperimentReconciler) acquireTarget(ctx context.Context, instance *v2beta1.Experiment) bool {
	log := Logger(ctx)
	log.Info("acquireTarget called")
	defer log.Info("acquireTarget completed")

	// do we already have the target?
	log.Info("acquireTarget", "Acquired", instance.Status.GetCondition(v2beta1.ExperimentConditionTargetAcquired))
	if instance.Status.GetCondition(v2beta1.ExperimentConditionTargetAcquired).IsTrue() {
		return true
	}
	...
	for _, e := range shareTarget {
		if !sameInstance(instance, e) {
			if e.Status.GetCondition(v2beta1.ExperimentConditionTargetAcquired).IsTrue() {
...
func (r *ExperimentReconciler) activeContendersForTarget(ctx context.Context, target string) []*v2beta1.Experiment {
	log := Logger(ctx)
	log.Info("activeContendersForTarget called")
	defer log.Info("activeContendersForTarget completed")

	result := []*v2beta1.Experiment{}

	experiments := &v2beta1.ExperimentList{}
	...
	for i := range experiments.Items {
		if experiments.Items[i].Spec.Target == target &&
			experiments.Items[i].Status.GetCondition(v2beta1.ExperimentConditionExperimentCompleted).IsFalse() {
...
func sameInstance(instance1 *v2beta1.Experiment, instance2 *v2beta1.Experiment) bool {
...
func (r *ExperimentReconciler) triggerWaitingExperiments(ctx context.Context, instance *v2beta1.Experiment) {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) nextWaitingExperiment(ctx context.Context, target string, instance *v2beta1.Experiment) *v2beta1.Experiment {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) triggerNextExperiment(ctx context.Context, target string, instance *v2beta1.Experiment) {
	log := Logger(ctx)
