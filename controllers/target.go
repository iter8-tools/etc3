...
func (r *ExperimentReconciler) acquireTarget(ctx context.Context, instance *v2alpha3.Experiment) bool {
	log := Logger(ctx)
	log.Info("acquireTarget called")
	defer log.Info("acquireTarget completed")

	// do we already have the target?
	log.Info("acquireTarget", "Acquired", instance.Status.GetCondition(v2alpha3.ExperimentConditionTargetAcquired))
	if instance.Status.GetCondition(v2alpha3.ExperimentConditionTargetAcquired).IsTrue() {
		return true
	}
	...
	for _, e := range shareTarget {
		if !sameInstance(instance, e) {
			if e.Status.GetCondition(v2alpha3.ExperimentConditionTargetAcquired).IsTrue() {
...
func (r *ExperimentReconciler) activeContendersForTarget(ctx context.Context, target string) []*v2alpha3.Experiment {
	log := Logger(ctx)
	log.Info("activeContendersForTarget called")
	defer log.Info("activeContendersForTarget completed")

	result := []*v2alpha3.Experiment{}

	experiments := &v2alpha3.ExperimentList{}
	...
	for i := range experiments.Items {
		if experiments.Items[i].Spec.Target == target &&
			experiments.Items[i].Status.GetCondition(v2alpha3.ExperimentConditionExperimentCompleted).IsFalse() {
...
func sameInstance(instance1 *v2alpha3.Experiment, instance2 *v2alpha3.Experiment) bool {
...
func (r *ExperimentReconciler) triggerWaitingExperiments(ctx context.Context, instance *v2alpha3.Experiment) {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) nextWaitingExperiment(ctx context.Context, target string, instance *v2alpha3.Experiment) *v2alpha3.Experiment {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) triggerNextExperiment(ctx context.Context, target string, instance *v2alpha3.Experiment) {
	log := Logger(ctx)
