...
func (r *ExperimentReconciler) ReadMetric(ctx context.Context, instance *v2beta1.Experiment, namespace string, name string, metricMap map[string]*v2beta1.Metric) bool {
	log := Logger(ctx)
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			r.recordExperimentFailed(ctx, instance, v2beta1.ReasonMetricUnavailable, "Unable to find metric %s/%s", namespace, name)
		} else {
			r.recordExperimentFailed(ctx, instance, v2beta1.ReasonMetricsUnreadable, "Unable to load metric %s/%s", namespace, name)
		}
		return false // not ok
	}
...
func shouldReadMetrics(instance *v2beta1.Experiment) bool {
...
func (r *ExperimentReconciler) ReadMetrics(ctx context.Context, instance *v2beta1.Experiment) bool {
	log := Logger(ctx)
	...
	metricsCache := make(map[string]*v2beta1.Metric)
	...
	for name, obj := range metricsCache {
		instance.Status.Metrics = append(instance.Status.Metrics,
			v2beta1.MetricInfo{Name: name, MetricObj: *obj})
	}
	return true
}
