...
func (r *ExperimentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if instance.Status.InitTime == nil {
		...
		r.recordExperimentProgress(ctx, instance,
			v2alpha3.ReasonExperimentInitialized, "Experiment status initialized")
		return r.endRequest(ctx, instance)
	}
	log.Info("Status initialized")

	// If experiment already completed, stop
	if instance.Status.GetCondition(v2alpha3.ExperimentConditionExperimentCompleted).IsTrue() {
	...
	if ok := r.advanceStage(ctx, instance, v2alpha3.ExperimentStageInitializing); ok {
	...
	// when we advance for the first time, we exit to force update; will be retriggered
	if ok := r.advanceStage(ctx, instance, v2alpha3.ExperimentStageRunning); ok {
	...
	// if instance.Spec.GetAlgorithm() == v2alpha3.AlgorithmTypeFixedSplit {
...
func (r *ExperimentReconciler) advanceStage(ctx context.Context, instance *v2alpha3.Experiment, to v2alpha3.ExperimentStageType) bool {
	log := Logger(ctx)
	...
	if to.After(stage) {
		...
		r.recordExperimentProgress(ctx, instance, v2alpha3.ReasonStageAdvanced, "Advanced to %s", to)
		return true
	}
	return false
}
...
func (r *ExperimentReconciler) endRequest(ctx context.Context, instance *v2alpha3.Experiment, interval ...time.Duration) (ctrl.Result, error) {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) endExperiment(ctx context.Context, instance *v2alpha3.Experiment, msg string) (ctrl.Result, error) {
	log := Logger(ctx)
	...
	if ok := r.advanceStage(ctx, instance, v2alpha3.ExperimentStageCompleted); ok {
...
func (r *ExperimentReconciler) finishExperiment(ctx context.Context, instance *v2alpha3.Experiment) (ctrl.Result, error) {
	log := Logger(ctx)
	...
	if stop, result, err := r.launchHandlerWrapper(ctx, instance, HandlerTypeFinish,
		handlerLaunchModifier{onSuccessfulLaunch: func() { r.advanceStage(ctx, instance, v2alpha3.ExperimentStageFinishing) }},
	); stop {
		return result, err
	}
...
func (r *ExperimentReconciler) rollbackExperiment(ctx context.Context, instance *v2alpha3.Experiment) (ctrl.Result, error) {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) failExperiment(ctx context.Context, instance *v2alpha3.Experiment, err error) (ctrl.Result, error) {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) updateStatus(ctx context.Context, instance *v2alpha3.Experiment) error {
	log := Logger(ctx)
...
func (r *ExperimentReconciler) checkHandlersStatus(ctx context.Context, instance *v2alpha3.Experiment,
...
func (r *ExperimentReconciler) checkHandlerStatus(ctx context.Context, instance *v2alpha3.Experiment,
	...
	case HandlerStatusFailed:
		...
		r.recordExperimentFailed(ctx, instance, v2alpha3.ReasonHandlerFailed, msg)
...
func (r *ExperimentReconciler) launchHandlerWrapper(
	ctx context.Context, instance *v2alpha3.Experiment, handlerType HandlerType,
	...
	// record launch
	r.recordExperimentProgress(ctx, instance, v2alpha3.ReasonHandlerLaunched, "%s handler '%s' launched", handlerType, *handler)
...
func (r *ExperimentReconciler) cleanupDeletedExperiments(ctx context.Context, instance *v2alpha3.Experiment) {
	log := Logger(ctx)
