...
var _ = Describe("Target Acquisition", func() {
	...
	Context("Experiment already has the target", func() {
		...
		JustBeforeEach(func() {
			experiment = v2beta1.NewExperiment("already-has-target", testNamespace).
				WithTarget("targettest1").
				WithTestingPattern(v2beta1.TestingPatternCanary).
				WithCondition(v2beta1.ExperimentConditionTargetAcquired, corev1.ConditionTrue, v2beta1.ReasonTargetAcquired, "").
				Build()
		})
	...
	Context("Experiment wanting to acquire a target", func() {
		var (
			...
			has, wants         *v2beta1.Experiment
		)
		JustBeforeEach(func() {
			hasName = "willget-target"
			has = v2beta1.NewExperiment(hasName, testNamespace).
			...
			wants = v2beta1.NewExperiment(wantsName, testNamespace).
				WithTarget("unavailable-target").
				WithTestingPattern(v2beta1.TestingPatternConformance).
				WithDuration(1, 1, 1).
				WithBaselineVersion("baseline", nil).
		...
		It("will acquire the target only after a target holder is completed", func() {
			...
			Eventually(func() bool {
				return hasValue(hasName, testNamespace, func(exp *v2beta1.Experiment) bool {
			...
			Eventually(func() bool {
				return hasValue(wantsName, testNamespace, func(exp *v2beta1.Experiment) bool {
			...
			Eventually(func() bool {
				return hasValue(wantsName, testNamespace, func(exp *v2beta1.Experiment) bool {
					return exp.Status.Stage != nil && *exp.Status.Stage == v2beta1.ExperimentStageWaiting
				})
			}, 3).Should(BeTrue())
			...
			Eventually(func() bool {
				return hasValue(hasName, testNamespace, func(exp *v2beta1.Experiment) bool {
					return *exp.Status.Stage == v2beta1.ExperimentStageCompleted
				})
			}, 10).Should(BeTrue())
			...
			Eventually(func() bool {
				return hasValue(wantsName, testNamespace, func(exp *v2beta1.Experiment) bool {
					return exp.Status.Stage != nil && (*exp.Status.Stage == v2beta1.ExperimentStageRunning || *exp.Status.Stage == v2beta1.ExperimentStageCompleted)
				})
			}, 5).Should(BeTrue())
			Eventually(func() bool { return completes(wantsName, testNamespace) }).Should(BeTrue())
			Eventually(func() bool {
				return hasValue(wantsName, testNamespace, func(exp *v2beta1.Experiment) bool {
					return exp.Status.Stage != nil && *exp.Status.Stage == v2beta1.ExperimentStageCompleted
				})
			}, 5).Should(BeTrue())
