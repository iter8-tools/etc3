...
var _ = Describe("Metrics", func() {
	...
	var goodObjectiveMetric, goodObjective2Metric, badObjectiveMetric, rewardMetric *v2alpha3.Metric
	BeforeEach(func() {
		testNamespace = "default"
		metricsNamespace = "metric-namespace"

		k8sClient.DeleteAllOf(ctx(), &v2alpha3.Experiment{}, client.InNamespace(testNamespace))
		k8sClient.DeleteAllOf(ctx(), &v2alpha3.Metric{}, client.InNamespace(testNamespace))
		k8sClient.DeleteAllOf(ctx(), &v2alpha3.Metric{}, client.InNamespace(metricsNamespace))

		By("Providing a request-count metric")
		m := v2alpha3.NewMetric("request-count", metricsNamespace).
		...
		goodObjective2Metric = v2alpha3.NewMetric("objective-with-good-reference-2", metricsNamespace).
			WithType(v2alpha3.CounterMetricType).
			WithParams([]v2alpha3.NamedValue{{
				Name:  "param",
				Value: "value",
		...
		goodObjectiveMetric = v2alpha3.NewMetric("objective-with-good-reference", "default").
			WithType(v2alpha3.CounterMetricType).
			WithParams([]v2alpha3.NamedValue{{
				Name:  "param",
				Value: "value",
		...
		badObjectiveMetric = v2alpha3.NewMetric("objective-with-bad-reference", "default").
			WithType(v2alpha3.CounterMetricType).
			WithParams([]v2alpha3.NamedValue{{
				Name:  "param",
				Value: "value",
		...
		rewardMetric = v2alpha3.NewMetric("rwrd", "default").
			WithType(v2alpha3.CounterMetricType).
			WithParams([]v2alpha3.NamedValue{{
				Name:  "param",
				Value: "value",
	...
	Context("When creating an experiment referencing valid metrics", func() {
		...
		It("Should successfully read the metrics and proceed", func() {
			...
			experiment := v2alpha3.NewExperiment(testName, testNamespace).
				...
				WithReward(*rewardMetric, v2alpha3.PreferredDirectionHigher).
				Build()
			Expect(k8sClient.Create(ctx(), experiment)).Should(Succeed())
			By("Checking that it starts Running")
			// this assumes that it runs for a while
			Eventually(func() bool {
				return containsSubString(events, "Advanced to Running") //v2alpha3.ReasonStageAdvanced)
			}, 5).Should(BeTrue())
		})
	})
	...
	Context("failed start handler", func() {
		Specify("experiment teminated in a failed state", func() {
			...
			handler := "start"
			experiment := v2alpha3.NewExperiment(name, testNamespace).
				WithTarget(target).
				WithTestingPattern(v2alpha3.TestingPatternConformance).
				WithAction(handler, []v2alpha3.TaskSpec{}).
				WithRequestCount(metricsNamespace+"/request-count").
	...
	Context("When creating an experiment referencing a metric with a bad reference", func() {
		...
		It("Should successfully read metrics", func() {
			...
			// this assumes that it runs for a while
			Eventually(func() bool {
				return containsSubString(events, v2alpha3.ReasonStageAdvanced)
			}, 5).Should(BeTrue())
		})
	})
